...
def perturbBG(opt,imageData):
	rot = opt.pertBG*tf.random.normal([opt.batchSize]) #pertBG default 값은 0.1, batchSize는 20, 20개의 행에 random.normal값 * 0.1
	tx = opt.pertBG*tf.random.normal([opt.batchSize])
	ty = opt.pertBG*tf.random.normal([opt.batchSize])
	O = tf.zeros([opt.batchSize]) #20개 행에 전부 0
