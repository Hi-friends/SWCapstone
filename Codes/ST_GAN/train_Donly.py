...
os.makedirs("models_{0}".format(opt.group),exist_ok=True) #group은 따로 지정하지 않으면 기본값인 0
...
with tf.device(opt.GPUdevice):
	...
	imageRealData = tf.placeholder(tf.float32,shape=[opt.batchSize,opt.dataH,opt.dataW,3]) #218x178
	imageBGfakeData = tf.placeholder(tf.float32,shape=[opt.batchSize,opt.dataH,opt.dataW,3])
	imageFGfake = tf.placeholder(tf.float32,shape=[opt.batchSize,opt.H,opt.W,4])#144x144
	PH = [imageBGfakeData,imageRealData,imageFGfake]
	# ------ generate perturbation ------ noise 임의로 만듬
	imageReal = data.perturbBG(opt,imageRealData)
	imageBGfake = data.perturbBG(opt,imageBGfakeData)
	pPertFG = opt.pertFG*tf.random.normal([opt.batchSize,opt.warpDim])
	# ------ define GP and D ------ geometric predictor & discriminator 정의
	...
	alpha = tf.random.uniform(shape=[opt.batchSize,1,1,1])
